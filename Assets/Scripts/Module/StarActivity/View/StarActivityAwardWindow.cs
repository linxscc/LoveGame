...
public class StarActivityAwardWindow : Window {
	...
	{
		var prefab = GetPrefab("StarActivity/Prefabs/StarActivityAwardItem");
		foreach (var t in awards)
		{
			var go = Instantiate(prefab, _parentTra, false);
			go.GetComponent<StarActivityAwardItem>().SetData(t);
		}
