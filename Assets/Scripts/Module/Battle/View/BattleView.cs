...
public class BattleView : View
{
    ...
    private void Start()
    {
        _battleCommon = transform.parent.Find("BattleCommon(Clone)");
    ...
    {
        //应援会能力值加到进度条动画
        SendMessage(new Message(MessageConst.CMD_BATTLE_SHOW_SUPPORTER_POWER));

        ClientTimer.Instance.DelayCall(() =>
        {
            SendMessage(new Message(MessageConst.CMD_BATTLE_SHOW_SUPPORTER_VIEW));
            _step++;
        }, 0.6f);
    }
...
//                tween.onComplete = () => { };
    ...
    private void TypingEnd()
    {
        ...
        PointerClickListener.Get(_fans.gameObject).onClick = go =>
        {
            ...
            {
                //过场动画之前先隐藏进度条
                SendMessage(new Message(MessageConst.CMD_BATTLE_HIDE_PROGRESS));
            ...
            {
                //显示选星缘
                ClientTimer.Instance.DelayCall(() => { SendMessage(new Message(MessageConst.CMD_BATTLE_NEXT)); }, 0.2f);
            }
        };
    }
    ...
    private void ShowMask()
    {
        ...
        _mask.DOLocalMoveX(-2200, 1.0f).onComplete = () =>
        {
            ...
            ClientTimer.Instance.DelayCall(
                () =>
                {
                    SendMessage(new Message(MessageConst.CMD_BATTLE_CHANGE_POWER, Message.MessageReciverType.DEFAULT,
                        _tempPower));
                }, 0.1f);

            //发送第二次显示粉丝Msg
            SendMessage(new Message(MessageConst.CMD_BATTLE_SECOND_SHOW_FANS));
        };
    }
    ...
    private void RoleImageAni()
    {
        if (_getRoleIds.Count == 0)
        {
            ...
            ClientTimer.Instance.DelayCall(() =>
            {
                SendMessage(new Message(MessageConst.CMD_BATTLE_FANS_CALL_ANIMATION_FINISH));
                return;
            }, 1.0f);
        }
        else
